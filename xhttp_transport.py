...
ترابرد XHTTP برای Straw Hat | 麦わら帽子 (LUFFY PANEL) — دو مد: packet-up / stream-up
...
async def _get_or_create_session(uid: str, auth: str, mode: str, session_id: str, ip: str) -> dict:
    # Held for the whole function, not just the initial lookup: XHTTP opens
    # its downlink GET and first uplink POST almost simultaneously for a
    # brand-new session, so two requests can race to create the same
    # session_id. Serializing creation here is cheap (everything below is
    # in-memory / other independent locks, no real I/O) and closes that
    # race — without it, the loser's connection-tracking entry would be
    # silently clobbered and leak forever (never torn down, permanently
    # inflating that link's connection count).
    ...
    # Notification can involve a real network call to Telegram (if enabled
    # in Settings) — done after releasing XHTTP_LOCK so a slow/unreachable
    # Telegram API can't stall every other new session's creation. The
    # race this lock exists to close is already resolved by the point the
    # dict write above happened.
    await _log_connection_event("connect", label, uid, ip)
    return sess
...
async def _pump_tcp_to_queue(session_id: str, uid: str, reader: asyncio.StreamReader, down_q: asyncio.Queue, resp_prefix: bytes = b"\x00\x00"):
    ...
    try:
        ...
        # Cleanup below still runs (finally), but re-raise so task
        # cancellation (e.g. graceful shutdown) actually propagates instead
        # of silently looking like this task finished normally.
        raise
    except Exception:
        pass
    finally:
